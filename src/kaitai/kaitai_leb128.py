...
class Leb128(ReadWriteKaitaiStruct):
    ...
    * Apache Avro uses this as a basis for integer encoding, adding ZigZag on
    ...
    def _read(self):
        ...
        i = 0
        while True:
            _t_groups = Leb128.Group(i, (self.groups[(i - 1)].interm_value if (i != 0) else 0), ((9223372036854775808 if (i == 9) else (self.groups[(i - 1)].multiplier * 128)) if (i != 0) else 1), self._io, self, self._root)
            _t_groups._read()
            _ = _t_groups
    ...
    def _write__seq(self, io=None):
        super(Leb128, self)._write__seq(io)
        for i in range(len(self.groups)):
            pass
    ...
    def _check(self):
        ...
        for i in range(len(self.groups)):
            ...
            if (self.groups[i].idx != i):
                raise kaitaistruct.ConsistencyError(u"groups", self.groups[i].idx, i)
            if (self.groups[i].prev_interm_value != (self.groups[(i - 1)].interm_value if (i != 0) else 0)):
                raise kaitaistruct.ConsistencyError(u"groups", self.groups[i].prev_interm_value, (self.groups[(i - 1)].interm_value if (i != 0) else 0))
            if (self.groups[i].multiplier != ((9223372036854775808 if (i == 9) else (self.groups[(i - 1)].multiplier * 128)) if (i != 0) else 1)):
                raise kaitaistruct.ConsistencyError(u"groups", self.groups[i].multiplier, ((9223372036854775808 if (i == 9) else (self.groups[(i - 1)].multiplier * 128)) if (i != 0) else 1))
            _ = self.groups[i]
    ...
    class Group(ReadWriteKaitaiStruct):
        ...
        def _write__seq(self, io=None):
            super(Leb128.Group, self)._write__seq(io)
